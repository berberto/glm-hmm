...
if __name__ == '__main__':
    ...
    # Load data
    inpt, y, session = load_data(f"{data_dir}/all_animals_concat.npz")
    ...
    for fold in range(num_folds):
        ...
        for model in models:
            ...
            if model == "GLM":
                ...
                # these parameters
                glm_weights_file = f"{results_dir}/GLM/fold_{str(fold)}/variables_of_interest_iter_0.npz"
                ...
                for K in range(2, K_max + 1):
                    ...
                    key_for_dict = f'/GLM_HMM_K_{str(K)}/fold_{str(fold)}'
    ...
    f = open(f"{results_dir}/best_init_cvbt_dict.json", "w")
    f.write(json_dump)
    f.close()
    # Save cvbt_folds_model as numpy array for easy parsing across all
    # models and folds
    np.savez(f"{results_dir}/cvbt_folds_model.npz", cvbt_folds_model)
    np.savez(f"{results_dir}/cvbt_train_folds_model.npz",
             cvbt_train_folds_model)
