...
if __name__ == '__main__':
    ...
    for animal in animal_list:
        ...
        for fold in range(num_folds):
            ...
            for model in models:
                ...
                if model == "GLM":
                    ...
                    glm_weights_file = f"{overall_dir}/GLM/fold_{str(fold)}/variables_of_interest_iter_0.npz"
        ...
        f = open(f"{overall_dir}/best_init_cvbt_dict.json", "w")
        f.write(json_dump)
        f.close()
        # Save cvbt_folds_model as numpy array for easy parsing
        # across all models and folds
        np.savez(f"{overall_dir}/cvbt_folds_model.npz", cvbt_folds_model)
        np.savez(f"{overall_dir}/cvbt_train_folds_model.npz",
                 cvbt_train_folds_model)